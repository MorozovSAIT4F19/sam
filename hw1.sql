...
-- 1) Открываем файл credit_events_task.xls с помощью Microsoft Office Excel.
-- 2) Переходим во вкладку «Файл». Нажимаем «Сохранить как». Выбираем текущую папку. Имя файла оставляем прежним,
-- а тип файла выбираем «CSV (разделители – запятые)».
-- 3) Закрываем файл credit_events_task.xls.
-- 4) Открываем файл ratings_task.xlsx с помощью Microsoft Office Excel.
-- 5) На листе actions кликаем на ячейку E2 и зажимаем Ctrl + Shift + ↓. Таким образом мы выделяем весь столбец 
-- значений date. Изменяем формат данного выделенного столбца на «Дата».
-- 6) Переходим во вкладку «Файл». Нажимаем «Сохранить как». Выбираем текущую папку. Имя файла оставляем прежним,
-- а тип файла выбираем «CSV (разделители – запятые)».
-- 7) Закрываем файл ratings_task.xlsx.

-- Данные необходимо хранить в папке, доступной всем пользователям (~\User\Public для Windows, /tmp для Mac), поэтому
-- сохраненные CSV файлы перемещаем туда.
---------------------------------------------------------------------------------------------------------
...
DROP TABLE if exists public.rat_info;
